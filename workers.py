...
from models import Category, FolderPlan, OtherCategory, TriageItem
...
class ScanWorker(_BaseWorker):
    ...
    def run(self) -> None:  # noqa: C901 - a linear pipeline reads better whole
        ...
        try:
            # ---- 1. Mail ------------------------------------------------
            self._emit_progress(0, 100, "Connecting to iCloud…")
            engine.connect(self.settings.icloud_email, self.icloud_password)
            self._log(
                f"Connected to {self.settings.imap_host} as {self.settings.icloud_email} "
                f"(hierarchy delimiter “{engine.delimiter}”)."
            )

            plan = engine.folder_plan(self.settings.folder_root, self.settings.other_folder_root)
            outcome.folder_plan = plan

            created = engine.ensure_folders(plan, subscribe=self.settings.subscribe_new_folders)
            outcome.created_folders = list(created)
            if created:
                self._log("Created folders: " + ", ".join(created))
            else:
                self._log(f"All “{plan.root}” folders already exist.")

            if not engine.has_capability("UIDPLUS"):
                outcome.warnings.append(
                    "This server does not advertise UIDPLUS, so applying moves will use a "
                    "full EXPUNGE of the source mailbox."
                )

            self._emit_progress(0, 100, "Searching for messages…")
            ...
            scan = engine.fetch_window(
                ...
                mailbox=self.settings.source_mailbox,
                max_messages=self.settings.max_messages,
                progress=fetch_report,
                cancel=self.cancel_event,
                connections=self.settings.imap_connections,
                max_bytes=self.settings.fetch_bytes,
            )
            outcome.warnings.extend(scan.warnings)
            outcome.scanned_count = len(scan.messages)
            self._log(
                f"Fetched {len(scan.messages)} message(s) from {self.settings.source_mailbox} "
                f"({len(scan.candidate_uids)} matched the server-side date search)."
            )
            if self.cancel_event.is_set():
            ...
            self._report_exception("Could not read your iCloud mailbox", exc)
            return
        finally:
            try:
                engine.logout()
...
class ApplyWorker(_BaseWorker):
    ...
    def run(self) -> None:
        engine = IMAPEngine(host=self.settings.imap_host, port=self.settings.imap_port)
        try:
            self._emit_progress(0, len(self.plans) or 1, "Connecting to iCloud…")
            engine.connect(self.settings.icloud_email, self.icloud_password)
            ...
            def apply_report(done: int, total: int, text: str) -> None:
                self._emit_progress(done, total, text)
                elapsed = max(1e-6, time.monotonic() - apply_started)
                self._emit_metrics(
                    phase="apply", done=done, total=total, elapsed=elapsed,
                    rate=done / elapsed,
                    eta=(total - done) / (done / elapsed) if done else 0.0,
                )
            ...
            report.created_folders = created
            self._log(f"Apply finished: {report.describe()}")
            self.finished_ok.emit(report)
        except ScanCancelled:
            self._log("Apply cancelled before completion.")
            self.finished_ok.emit(MoveReport(warnings=["Cancelled before all moves completed."]))
        except Exception as exc:  # noqa: BLE001
            self._report_exception("Could not file your messages", exc)
        finally:
            try:
                engine.logout()
            except Exception:  # pragma: no cover
...
class ConnectionTestWorker(_BaseWorker):
    ...
    def run(self) -> None:
        try:
            if self.mode == "imap":
                engine = IMAPEngine(host=self.settings.imap_host, port=self.settings.imap_port)
                result = engine.probe(self.settings.icloud_email, self.icloud_password)
                result["mode"] = "imap"
            else:
                classifier = LLMEngine(
                    provider=self.settings.provider,
...
def build_move_plans(items: Sequence[TriageItem]) -> List[MovePlan]:
    ...
    return [
        MovePlan(uid=item.email.uid, target_folder=item.target_folder, subject=item.email.subject_display)
        for item in items
